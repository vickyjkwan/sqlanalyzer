...
def get_table_names(line_query):
    """
...
def get_queried_columns(table_names, meta_cols):
    """
    ...
    for _,table_name in table_names.items():
        ...
        if len(table_name.split('.')) == 2:
            queried_cols.append({table_name: set(meta_cols.filter(meta_cols['db_table']==table_name).collect()[0][1])})

    return queried_cols
        

def map_db_columns(var_list, queried_cols, table_alias_mapping):
    """
...
def get_all_scanned_cols(cte_queries, meta_cols):
    ...
    for _,cte_query in cte_queries.items():
        ...
        table_alias_mapping = get_table_names(cte_query.split('\n'))
        variables = get_all_variables(cte_query)
        queried_columns = get_queried_columns(table_alias_mapping, meta_cols)
        if variables == []:
            original_columns_list = []
            ...
            original_columns_list = map_db_columns(variables, queried_columns, table_alias_mapping)
